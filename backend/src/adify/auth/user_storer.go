...
func (UserStorer) Get(Claims *claims.Claims, context *Context) (user interface{}, err error) {
	...
	if context.Auth.Config.UserModel != nil {
		if Claims.UserID != "" {
			currentUser := reflect.New(ModelType(context.Auth.Config.UserModel)).Interface()
	...
	var (
		authIdentity = reflect.New(ModelType(context.Auth.Config.AuthIdentityModel)).Interface()
...
func (UserStorer) Save(schema *Schema, context *Context) (user interface{}, userID string, err error) {
	...
	if context.Auth.Config.UserModel != nil {
		currentUser := context.Auth.Config.UserModel
